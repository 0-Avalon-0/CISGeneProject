...
test_data = pd.HDFStore('C:\dataset\\train_data.h5')
...
# encode train array
temp_matrix = np.zeros(shape=[21389, 512])
encode_array = tf.placeholder(dtype=tf.float32, shape=[21389, 512], name='encode_array')
encode_g2_variable = tf.Variable(tf.zeros([21389, 512]), dtype=tf.float32, name='encode_g2_variable')
encode = tf.assign(encode_g2_variable, encode_array)
# encode test array
...
# define saver
var_dict = {'encode_g2_variable': encode_g2_variable}
...
with tf.Session() as sess:
    ...
    save_path = saver.save(sess, 'C:\\Users\\11201\\Desktop\\parameter2\\save_net.ckpt')
